		...
		for (const auto& extension : physicsExtensions) {
			if (const auto& physicsExtension = extension.lock()) 
			{
				...
				std::function<void(const std::shared_ptr<spic::GameObject>&, const std::shared_ptr<spic::Collider>&)> stayCallback 
					= [this](const std::shared_ptr<spic::GameObject>& entity, const std::shared_ptr<spic::Collider>& collider) { OnStay(entity, collider); };
